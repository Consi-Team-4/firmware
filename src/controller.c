...
void feedback(TimerHandle_t xTimer)
{
    ...
    // this will have to change somehow, not sure
    LidarData lidarData;
    ...
    if (suspensionFeedbackEnable)
    {
        ...
        suspensionFeedback(suspensionData + SERVO_FR, dt, zP - zR, imuFiltered.Vz + vzP - vzR, lidarData);
        suspensionFeedback(suspensionData + SERVO_FL, dt, zP + zR, imuFiltered.Vz + vzP + vzR, lidarData);
        suspensionFeedback(suspensionData + SERVO_BR, dt, -zP - zR, imuFiltered.Vz - vzP - vzR, lidarData);
        suspensionFeedback(suspensionData + SERVO_BL, dt, -zP + zR, imuFiltered.Vz - vzP + vzR, lidarData);
    }
}
...
#define maxError 2
    ...
    if (escIntegral > maxIntegral)
    {
        escIntegral = maxIntegral;
    }
    else if (escIntegral < minIntegral)
    {
        escIntegral = minIntegral;
    }
...
#define maxD 1
    ...
    if (data->integral > maxIntegral)
    {
        data->integral = maxIntegral;
    }
    if (data->integral < minIntegral)
    {
        data->integral = minIntegral;
    }

    data->output = data->neutralPosition + suspensionKP * (-z) + data->integral + suspensionKD * (-vz);

    if (data->output > limits.maxPosition)
    {
        data->output = limits.maxPosition;
    }
    else if (data->output < limits.minPosition)
    {
        data->output = limits.minPosition;
    }
