...
void lidarSetup()
{
    ...
    //     // Read 9 bytes from second LiDAR
    //     // for (int i = 0; i < 9; i++) {
    //     //   data_2[i] = Serial2.read();
    //     //   if (i < 8) checksum += data_2[i]; // Calculate checksum for first 8 bytes
    //     // }

    //     // Verify frame headers and checksum for first LiDAR

    //     uint16_t distance = (lidar0Buffer[3] << 8) + lidar0Buffer[2]; // Combine high and low bytes
    //     printf("%d\n", distance);

    //     // if (distance < 10)
    //     // {
    //     //     // adjustServoPosition(2500, distance);
    //     // }
    //     // if (distance > 80)
    //     // {
    //     //     // adjustServoPosition(500, distance);
    //     // }
    // }
